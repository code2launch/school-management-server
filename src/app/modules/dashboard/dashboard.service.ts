...
const getAdminDashboard = async () => {
  ...
  const [
    ...
    // Fee collection this month (using paymentDate)
    prisma.feePayment.aggregate({
      where: {
        ...(academicYearId ? { academicYearId } : {}),
        paymentStatus: "PAID", // Only count fully paid payments
        paymentDate: {
          gte: monthStart,
          lte: monthEnd,
        },
    ...
    // Fee collection this year (for comparison)
    prisma.feePayment.aggregate({
      where: {
        ...(academicYearId ? { academicYearId } : {}),
        paymentStatus: "PAID",
    ...
    // Total pending dues amount (including partial payments)
    prisma.feePayment.aggregate({
      where: {
        ...(academicYearId ? { academicYearId } : {}),
        paymentStatus: { in: ["UNPAID", "PARTIAL"] },
        dueAmount: { gt: 0 }, // Only where due amount is greater than 0
      },
      _sum: { dueAmount: true },
      _count: true,
    }),
  ...
  const totalAttendance = Object.values(attCounts).reduce((s, v) => s + v, 0);
  ...
  return {
    ...
    stats: {
      ...
      todayAttendance: {
        ...
        attendanceRate:
          totalAttendance > 0
            ? ((attCounts["PRESENT"] ?? 0) / totalAttendance) * 100
            : 0,
      },
      feeCollectedThisMonth: feeCollectionThisMonth._sum.paidAmount ?? 0,
      feeCollectedThisYear: feeCollectionThisYear._sum.paidAmount ?? 0,
      totalPendingDue: pendingDues._sum.dueAmount ?? 0,
      studentsWithDues: pendingDues._count,
    },
    enrollmentBreakdown,
...
const getTeacherDashboard = async (teacherId: string) => {
  ...
  const [teacher, todaysClasses, subjectAssignments, teacherAttToday] =
    await Promise.all([
  ...
  const sectionAttendanceSummary = await Promise.all(
    sectionIds.map(async (sectionId) => {
      ...
      const section = todaysClasses.find(
...
const getStudentDashboard = async (studentId: string) => {
  ...
  const [attendanceSummary, feeStatus, recentResults, todayRoutine, notices] =
    await Promise.all([
  ...
  const totalAttDays = Object.values(attCounts).reduce(
    (s: number, v: any) => s + v,
    0,
  );
  const presentDays = attCounts["PRESENT"] ?? 0;

  const totalDue = (feeStatus as any[]).reduce(
    (s: number, f: any) => s + f.dueAmount,
    0,
  );
  ...
  return {
    ...
    attendanceSummary: {
      ...
      total: totalAttDays,
      percentage:
        totalAttDays > 0 ? Math.round((presentDays / totalAttDays) * 100) : 0,
    },
    feeStatus: {
...
const getParentDashboard = async (parentId: string) => {
  ...
  const childrenData = await Promise.all(
    parent.children.map(async ({ student }) => {
      ...
      const totalDays = Object.values(attCounts).reduce(
        (s: number, v: any) => s + v,
        0,
      );
      ...
      return {
        ...
        monthlyAttendance: {
          ...
          percentage:
            totalDays > 0
              ? Math.round(((attCounts["PRESENT"] ?? 0) / totalDays) * 100)
              : 0,
        },
        fees: {
